...
def complementarity():
    ...
    for label, n, color in segs:
        ...
        ax.text(left + n / 2, 0, f'{label}\n{n}',
                ha='center', va='center', color='black' if color in dark_label else 'white',
                fontsize=9)
        left += n
    ax.set_xlim(0, total)
    ax.set_ylim(-0.5, 0.5)
...
def operating_curve():
    ...
    ax.plot(crop_fp, crop_r, '-^', color=C_CROP, mfc='white', ms=5, lw=1.3,
            label='Crop branch only')
    ax.plot(yolo_fp, yolo_r, '-s', color=C_MISS, mfc='white', ms=5, lw=1.3,
            label='YOLO branch only')
    ax.plot(comb_fp, comb_r, '-o', color=C_YOLO, ms=5, lw=1.5,
    ...
    for x, y, t in zip(comb_fp, comb_r, thr):
        ax.annotate(f'{t:.2f}', (x, y), textcoords='offset points',
                    xytext=(0, 7), ha='center', fontsize=7, color='#11406b')

    # Mark the deployed operating point.
    ax.plot(comb_fp[deployed], comb_r[deployed], 'o', ms=13, mfc='none',
            mec='#222222', mew=2)
    ax.annotate('deployed\n(conf 0.20)', (comb_fp[deployed], comb_r[deployed]),
                textcoords='offset points', xytext=(34, -22), ha='left',
...
def iteration_progression():
    ...
    colors = {'Iteration 1': '#9ecae1', 'Iteration 2': '#4292c6', 'Iteration 3': '#08519c'}
    ...
    for i, (it, vals) in enumerate(data.items()):
        ...
        bars = ax.bar(offsets, heights, bar_w, label=it, color=colors[it],
                      edgecolor='white')
        for off, v in zip(offsets, vals):
            if v is None:
                ax.text(off, 0.02, 'n/a', ha='center', va='bottom', fontsize=8,
                        rotation=90, color='grey')
            else:
                ax.text(off, v + 0.012, f'{v:.2f}', ha='center', va='bottom', fontsize=8)
...
def class_imbalance():
    ...
    counts = [('fly', 3636, C_YOLO), ('other', 526, C_CROP),
              ('butterfly', 165, C_BOTH), ('bumblebee', 14, C_MISS)]
    ...
    ax.set_ylim(0, 3950)
    ax.text(3, 220, 'excluded from\nYOLO (too few)', ha='center', va='bottom',
...
def pipeline_diagram():
    ...
    ax.text(-1.45, 5.55, 'YOLO branch', ha='center', fontsize=8.5,
            color=C_YOLO, style='italic')
    ax.text(1.45, 5.55, 'Crop branch', ha='center', fontsize=8.5,
            color='#9a5a00', style='italic')
...
def slicing_schematic():
    ...
    for x0 in xs:
        for y0 in ys:
            ax.add_patch(Rectangle((x0, y0), tile, tile, facecolor=C_YOLO,
                                   alpha=0.13, edgecolor=C_YOLO, lw=0.5))

    # Label one tile and one overlap band.
    ax.annotate('one 640\\,px tile'.replace('\\,', ' '), xy=(tile / 2, tile * 0.12),
                ha='center', fontsize=8, color='#11406b')
    band_cx = xs[1] + (tile - stride) / 2   # centre of the first vertical overlap band
    ax.annotate('20% overlap', xy=(band_cx, H * 0.9), xytext=(band_cx, H + 150),
    ...
    ax.add_patch(Rectangle((ix, iy), iw, ih, facecolor='#cc3333', edgecolor='black', lw=0.6))
    ax.annotate('insect on a tile seam\n(appears in both tiles, merged by SAHI)',
                xy=(ix + iw / 2, iy + ih / 2), xytext=(W * 0.42, H * 0.55),
                fontsize=9, ha='left', va='center',
                arrowprops=dict(arrowstyle='->', lw=1))

    ax.text(W / 2, -110, 'source frame (schematic; 640\\,px tiles at 20% overlap)'.replace('\\,', ' '),
