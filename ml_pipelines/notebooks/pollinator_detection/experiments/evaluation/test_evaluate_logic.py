...
def center_match(pred_boxes, gt_boxes, overlap_thresh=0.20):
    ...
    for gi, g in enumerate(gt_boxes):
        ...
        for pi, p in enumerate(pred_boxes):
            ...
            pcy = (p['y1'] + p['y2']) / 2
            if (
                inside(gcx, gcy, p['x1'], p['y1'], p['x2'], p['y2'])
                or inside(pcx, pcy, g['x1'], g['y1'], g['x2'], g['y2'])
                or bbox_overlap_ratio(p, g) >= overlap_thresh
            ):
                pairs.append((pi, gi))
                mp.add(pi)
                mg.add(gi)
...
def evaluate_one_pipeline(preds_by_img, gt, classes, label):
    ...
    for img_p, gt_boxes in gt.items():
        ...
        for pi in unp:
            det_fp += 1
            fp_c[insect[pi]['cls']] += 1
            rows_out.append(
                {
                    'pipeline': label,
                    'img': img_p,
                    'match': 'fp',
                    'pred_cls': insect[pi]['cls'],
                    'gt_cls': '',
                    'conf': insect[pi]['conf'],
                    'correct_cls': False,
                }
    ...
    for img_p, gt_boxes in gt_by_img.items():
        ...
        for pi, gi in pairs:
            ...
            if pc == gc:
                ...
                cls_fn[gc] += 1
        for pi in unp:
            cls_fp[insect[pi]['cls']] += 1
        for gi in ung:
            cls_fn[gt_boxes[gi]['cls']] += 1
    results = {}
...
def combined_recall(crop_preds, yolo_preds, gt):
    ...
    for img_p, gt_boxes in gt.items():
        ...
        for gi in range(len(gt_boxes)):
            ic = gi in crop_matched
            iy = gi in yolo_matched
            if ic and iy:
                both += 1
            elif ic:
                crop_only += 1
...
gt = [
    gt_box(75, 0, 175, 100)
]  # GT area=100*100=10000; inter=[75,100]*[0,100]=25*100=2500; ratio=0.25 ≥ 0.20
pairs, unp, ung = center_match(pred, gt)
...
fly_tp = r['tp_c'].get('fly', 0)
fly_fp = r['fp_c'].get('fly', 0)
fly_fn = r['fn_c'].get('fly', 0)
...
fly_fp = r['fp_c'].get('fly', 0)
fly_fn = r['fn_c'].get('fly', 0)
fly_P = fly_tp / max(1, fly_tp + fly_fp)
fly_R = fly_tp / max(1, fly_tp + fly_fn)
...
for c in CLASSES:
    tp_g = r_global['tp_c'].get(c, 0)
    fp_g = r_global['fp_c'].get(c, 0)
    fn_g = r_global['fn_c'].get(c, 0)
    tp_p = pp[c]['tp']
    fp_p = pp[c]['fp']
    fn_p = pp[c]['fn']
...
check(
    'per-plot: butterfly→other error recorded',
    pp['butterfly']['errors'].get('other', 0),
    1,
)
...
crop_all = {
    'img1': [
        box(10, 10, 90, 90, 'fly'),
        box(210, 210, 290, 290, 'fly'),
        box(410, 410, 490, 490, 'fly'),
    ]
}
...
def threshold_sweep(rows):
    ...
    dets = sorted(
        [
            (ro['conf'], ro['match'] == 'tp')
            for ro in rows[0]['rows']
            if ro['match'] in ('tp', 'fp')
        ],
        key=lambda x: -x[0],
    )
    tp_ = fp_ = 0
...
p1 = box(0, 0, 100, 100)
p2 = box(10, 10, 90, 90)  # p2 has higher IoU with GT
...
yolo_bb = {
    'img1': [box(10, 10, 90, 90, 'fly')]
}  # YOLO says fly (bbox overlaps GT bumblebee)

res = combined_perclass(crop_bb, yolo_bb, gt_bb, ['bumblebee', 'fly'])

check(
    "cls15: YOLO 'fly' at bumblebee → cls_yolo[bumblebee]=0",
    res['cls_yolo']['bumblebee'],
    0,
)
check(
    "cls15: YOLO 'fly' at bumblebee → cls_yolo[fly]=0 (GT is bumblebee, not fly)",
    res['cls_yolo'].get('fly', 0),
    0,
)
check(
    'cls15: crop correctly predicts bumblebee → cls_crop[bumblebee]=1',
    res['cls_crop']['bumblebee'],
    1,
)
check(
    'cls15: union bumblebee = 1 (crop got it right)', res['cls_union']['bumblebee'], 1
)
check(
    'cls15: overall both=1 (class-agnostic, bbox overlaps)', res['both'], 1
)  # location-agnostic total: yes both bboxes cover GT
...
check(
    'cls15: both correctly predict fly → cls_union[fly]=1', res2['cls_union']['fly'], 1
)
...
check(
    "cls15: YOLO 'other' at fly location → cls_yolo[fly]=0", res3['cls_yolo']['fly'], 0
)
check('cls15: union fly=1 (crop covers it)', res3['cls_union']['fly'], 1)
check(
    "cls15: cls_both fly=0 (YOLO didn't classify correctly)", res3['cls_both']['fly'], 0
)
...
gt_c = {
    'img1': [gt_box(0, 0, 100, 100, 'bumblebee'), gt_box(200, 200, 300, 300, 'fly')]
}
crop_c = {'img1': [box(210, 210, 290, 290, 'fly')]}  # crop: fly ✓
yolo_c = {
    'img1': [
        box(10, 10, 90, 90, 'fly'),  # YOLO at bumblebee → 'fly' ✗
        box(210, 210, 290, 290, 'fly'),
    ]
}  # YOLO at fly → 'fly' ✓
...
check(
    "cls16: bumblebee 'neither' (YOLO wrong label, crop missed)",
    pc['bumblebee']['neither'],
    1,
)
check(
    "cls16: bumblebee 'yolo_only' = 0 (YOLO labeled it fly)",
    pc['bumblebee']['yolo_only'],
    0,
)
...
gt_c2 = {
    'img1': [gt_box(0, 0, 100, 100, 'fly'), gt_box(200, 200, 300, 300, 'butterfly')]
}
...
check(
    'cls16: no crop_only/yolo_only/neither',
    pc2['_total']['crop_only'] + pc2['_total']['yolo_only'] + pc2['_total']['neither'],
    0,
)
