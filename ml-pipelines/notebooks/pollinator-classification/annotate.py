...
OUTPUT_DIR = (
    args.output.with_name('annotated_crops')
...
def hit_debug_overlay_bbox(task_idx: int, x: int, y: int):
    ...
    for i, (_, crop_fn) in enumerate(crops):
        ...
        if (
            dx - PAD_HIT <= x <= dx + dw + PAD_HIT
            and dy - PAD_HIT <= y <= dy + dh + PAD_HIT
        ):
            area = max(1, dw * dh)
    ...
    if DEBUG_TOP <= y < DEBUG_BOTTOM:
        ...
        if key_dp in debug_cache:
            ...
            for i, (_, crop_fn) in enumerate(crops):
                ...
                if (
                    dx - PAD_HIT <= x <= dx + dw + PAD_HIT
                    and dy - PAD_HIT <= local_y <= dy + dh + PAD_HIT
                ):
                    click_buf[0] = ('bbox', i)
                    return
        click_buf[0] = -2
...
def batch_label_or_undo(
    ...
    matching = [crop_fn for _, crop_fn in crops if progress.get(crop_fn) == label]
    if matching:
...
def render():
    ...
    page_txt = (
        f'Page {state["scroll_row"] + 1}/{max(1, total_rows)}'
        if total_rows
        else 'Page 0/0'
    )
    cv2.putText(
            ...
            if sel is not None and sel < len(crops):
                ...
                if progress.get(crop_fn) == label:
                    ...
                    if sel_row >= state['scroll_row'] + ROWS_VISIBLE:
                        state['scroll_row'] = min(
                            sel_row - ROWS_VISIBLE + 1, max_scroll
                        )
