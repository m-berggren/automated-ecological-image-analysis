...
def get_thumb(path):
    ...
    if img is None:
        ...
        cv2.putText(thumb, "?", (THUMB_SIZE//2 - 8, THUMB_SIZE//2 + 8),
...
def render(labeled_dir, crops):
    ...
    if not crops:
        ...
        if n_rows > visible_rows:
            ...
            bar_y = int(state["scroll_row"] / max(1, n_rows - visible_rows) * (GRID_H - bar_h))
            cv2.rectangle(canvas, (WIN_W - 8, bar_y),
                          (WIN_W - 2, bar_y + bar_h), (100, 100, 100), -1)
    ...
    btn_labels = [("b","background"),("1","bumblebee"),("2","fly"),
            ...
            for i in range(len(FOLDERS)):
                ...
                if y0 <= y <= y0 + 46:
                    if i != state["folder_idx"]:
                        ...
                        threading.Thread(
                            target=lambda: [get_thumb(p) for p in list_crops(labeled_dir / FOLDERS[i])[:60]],
                            daemon=True
                        ).start()
                    return
...
def main():
    ...
    parser.add_argument("--labeled", type=Path,
                        default=Path("Insects_images/annotated_crops/labeled"))
    args = parser.parse_args()
    labeled_dir = args.labeled.resolve()

    if not labeled_dir.exists():
        print(f"ERROR: not found: {labeled_dir}", file=sys.stderr)
        print(f"Usage: python relabel.py --labeled /full/path/to/labeled", file=sys.stderr)
        return 1
    ...
    while True:
        ...
        if ch == "a" or key in LEFT_KEYS:
            if crops:
                sel = state["selected_idx"]
                state["selected_idx"] = max(0, (sel-1) if sel is not None else len(crops)-1)

        elif ch == "d" or key in RIGHT_KEYS:
            if crops:
                sel = state["selected_idx"]
                state["selected_idx"] = min(len(crops)-1, (sel+1) if sel is not None else 0)
