...
def draw_bbox_overlay(img, crops, bbox_map, crops_labels, sel, scale, ox, oy,
                      ...
                      show_background_boxes=False, draw_x=0):
    ...
    if show_all_numbers:
        for i, (_, crop_fn) in enumerate(crops):
            ...
            cv2.rectangle(img, (nx - 1, ny - th_n - 2), (nx + tw_n + 2, ny + 2), (35, 35, 35), -1)
...
def get_selectable_crop_indices(task_idx=None):
    ...
    if state.get("skip_labeled_crops", False):
        _, _, crops, _, _ = tasks[state["task_idx"] if task_idx is None else clamp_task_idx(task_idx)]
...
def render_footer(canvas, g_done, total_rows):
    ...
    add_footer_button(canvas, x, row1, 140, btn_h, "nav_mode_toggle",
                      f"Nav: {'CROP' if nav == 'crop' else 'IMAGE'}", nav == "crop")
    x += 140 + gap
    label_all_on = state.get("batch_mode", False)
    add_footer_button(canvas, x, row1, 126, btn_h, "batch_mode",
                      "Label all: ON" if label_all_on else "Label all: OFF", label_all_on)
    x += 126 + gap
    ...
    add_footer_button(canvas, x, row1, 112, btn_h, "after_label_action",
                      f"After: {after_text}", after_key != "unselect")
    x += 112 + gap
    ...
    put_fitted_text(canvas, status, x + 10, row1 + btn_h - 8,
                    WIN_W - x - 20, 0.38, (160, 160, 160), 1)

    # ── Row 2 ─────────────────────────────────────────────────────────────────
    x = x0 + 10
    add_footer_button(canvas, x, row2, 102, btn_h, "show_bbox_labels", "Debug labels",
                      state.get("show_bbox_labels", True))
    x += 102 + gap
    add_footer_button(canvas, x, row2, 92, btn_h, "show_main_bbox_numbers", "Debug nums",
                      state.get("show_main_bbox_numbers", False))
    x += 92 + gap
    add_footer_button(canvas, x, row2, 102, btn_h, "show_labeled_boxes", "Debug boxes",
    ...
    x += 88 + gap
    add_footer_button(canvas, x, row2, 144, btn_h, "click_bbox_preview", "Debug click opens",
    ...
    put_fitted_text(canvas, hint, x0 + 10, WIN_H - 10, WIN_W - 20,
                    0.38, (145, 145, 145), 1)
...
def toggle_footer_control(key):
    ...
    if key == "label_tint_down":
        state["label_tint_percent"] = max(0, int(state.get("label_tint_percent", DEFAULT_LABEL_TINT_PERCENT)) - LABEL_TINT_STEP_PERCENT)
        save_session(state["task_idx"])
        return
    if key == "label_tint_up":
        state["label_tint_percent"] = min(MAX_LABEL_TINT_PERCENT, int(state.get("label_tint_percent", DEFAULT_LABEL_TINT_PERCENT)) + LABEL_TINT_STEP_PERCENT)
        save_session(state["task_idx"])
        return
...
def remember_single_label(idx, crop_fn):
    state["last_single_label"] = {"task_idx": state["task_idx"], "idx": idx, "crop_fn": crop_fn}
...
def on_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEWHEEL:
        ...
        state["scroll_row"] = max(0, min(state["scroll_row"] + (-1 if signed > 0 else 1), max_scroll))
        return
...
def batch_label_or_undo(state, label, tasks, progress, labeled_dir, save_progress, apply_label):
    _, _, crops, _, _ = tasks[state["task_idx"]]
...
def render():
    ...
    more_txt  = "  [MORE CROPS]" if total_rows > state["scroll_row"] + ROWS_VISIBLE else ""
    title = (f"[{idx+1}/{total_images}]  {cam_name} / {Path(img_name).name}"
             f"  ({done_this}/{len(crops)} labeled){more_txt}")
    cv2.putText(canvas, title, (10, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (220, 220, 220), 1)
    ...
    if nav == "crop":
        ...
        cv2.putText(canvas, badge_txt, (bx1 + 8, th_b + 9),
    ...
    while cv2.getTextSize(instruction, cv2.FONT_HERSHEY_SIMPLEX, instr_fs, 1)[0][0] > WIN_W - 20:
        ...
        if instr_fs <= 0.34:
            break
    cv2.putText(canvas, instruction, (10, 62), cv2.FONT_HERSHEY_SIMPLEX, instr_fs, (180, 180, 180), 1)
    ...
    for display_pos, i in enumerate(visible_indices):
        ...
        if label is not None and clip > 0:
            tint_alpha = max(0, min(MAX_LABEL_TINT_PERCENT,
                int(state.get("label_tint_percent", DEFAULT_LABEL_TINT_PERCENT)))) / 100.0
            if tint_alpha > 0:
                roi  = canvas[ty:ty + clip, tx:tx + th_w]
                tint = np.full_like(roi, color)
                canvas[ty:ty + clip, tx:tx + th_w] = cv2.addWeighted(tint, tint_alpha, roi, 1.0 - tint_alpha, 0)
...
def go_to_image(new_idx, overlay=None, overlay_kind=None):
    ...
    state.update({
        "task_idx": new_idx, "selected_idx": None, "scroll_row": 0,
        "last_batch": None, "last_single_label": None,
        "overlay": overlay, "overlay_kind": overlay_kind,
    })
    ...
    # ── Arrow keys ────────────────────────────────────────────────────────────
        ...
        if nav == "crop":
            new_sel = pick_adjacent_crop_index(
                (state["scroll_row"] * CROP_COLS) if state["selected_idx"] is None
                else min(state["selected_idx"] + CROP_COLS,
