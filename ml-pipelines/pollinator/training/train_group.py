...
def collect_samples(
    arctic_dir: Optional[Path], web_dir: Optional[Path], classes: list
) -> tuple:
    ...
    return (
        arctic_samples,
        web_samples,
        count_by_class(arctic_samples),
        count_by_class(web_samples),
    )


def stratified_split(
    samples: list, n_classes: int, val_frac: float, seed: int, test_frac: float = 0.1
) -> tuple:
...
def make_loader(
    samples: list, indices: list, img_size: int, batch: int, augment: bool = True
) -> DataLoader:
...
def run_stage(
    ...
    for ep in range(1, epochs + 1):
        ...
        if emit is not None:
            emit(
                ep,
                f'{name} epoch {ep}/{epochs}: tr_loss={tr_loss:.4f} '
                f'val_macro_f1={v["macro_f1"]:.3f} val_acc={v["acc"]:.3f}',
            )
...
def train_group(
    ...
    stages. Signature: (processed, total, message, level) where total is the
    sum of stage 1 + stage 2 epochs that will actually run, and processed is
    monotonic across stages. Exceptions raised by the callback are swallowed.
    """
    ...
    epochs_s1_used = epochs_s1 if stage1_active else 0
    total_epochs = epochs_s1_used + epochs_s2

    stage1_ckpt = out_dir / f'group_{model_type}_stage1.pth'
    if stage1_active:
        combined_samples = arctic_samples + web_samples
        combined_train_idx = arctic_train_idx + [n_arctic + i for i in web_train_idx]
        combined_counts = {
            i: counts_arctic[i] + counts_web[i] for i in range(len(CLASSES))
        }
        train_loader_s1 = make_loader(
            combined_samples, combined_train_idx, img_size, batch
        )
        model = run_stage(
            model,
            f'{model_type}_stage1_combined',
            train_loader_s1,
            val_loader,
            epochs_s1,
            lr_s1,
    ...
    if epochs_s2 == 0 and stage1_active:
        ...
        model = run_stage(
            ...
            device,
            CLASSES,
            emit=lambda ep, msg: _emit(epochs_s1_used + ep, total_epochs, msg),
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model', default='insectnet', choices=['efficientnet', 'insectnet']
    )
    parser.add_argument('--insectnet_weights', help='Path to InsectNet model.pth')
    parser.add_argument(
        '--no_unfreeze',
        action='store_true',
        help='Keep InsectNet backbone fully frozen',
    )
