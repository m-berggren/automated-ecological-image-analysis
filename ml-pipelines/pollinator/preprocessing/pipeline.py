...
def run_preprocessing(
    ...
    if skip_first_n > 0:
        ...
        logger.info(
            f'Skipping first {skip_first_n} of {n_total} images; processing {len(paths)} remaining'
        )
    if not paths:
        logger.warning(
            f'No images to process in {image_dir} (after skip_first_n={skip_first_n})'
        )
        return {
    ...
    for idx, path in enumerate(paths):
        ...
        if prev_dt is not None and cur_dt is not None:
            ...
            if gap > max_gap_sec:
                logger.info(
                    f'EXIF gap {gap:.0f}s exceeds {max_gap_sec:.0f}s; resetting background reference'
                )
                prev_frame = None
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Arctic pollinator preprocessing pipeline'
    )
    ...
    parser.add_argument(
        '--no_large_motion', action='store_true', help='Disable large motion tiling'
    )
    parser.add_argument(
        '--skip_first_n',
        type=int,
