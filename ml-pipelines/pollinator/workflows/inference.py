...
def run_insectnet_step(
    ...
    crop_paths = sorted(prep_crop_dir.glob('*.jpg')) + sorted(
        prep_crop_dir.glob('*.png')
    )
    if not crop_paths:
    ...
    logger.info(
        f'Binary: {n_kept}/{n_total} crops kept as insect (threshold={binary_threshold})'
    )
...
def run_pipeline(
    ...
    if skip_first_n > 0:
        ...
        logger.info(
            f'Skipping first {skip_first_n} of {n_total} images; processing {len(image_paths)} remaining'
        )
    if not image_paths:
        logger.warning(
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run full pollinator detection pipeline'
    )
