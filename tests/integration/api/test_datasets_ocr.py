...
def _post(client, upload):
    return client.post(
        IMAGES,
        {'file': _png(), 'module': 'seeds', 'purpose': 'inference', 'upload': upload.pk},
        format='multipart',
    )


def test_ocr_finds_species_accepts(auth_client, settings, tmp_path, monkeypatch, seeds_upload):
...
def test_ocr_misses_species_rejects(auth_client, settings, tmp_path, monkeypatch, seeds_upload):
    settings.MEDIA_ROOT = str(tmp_path)
